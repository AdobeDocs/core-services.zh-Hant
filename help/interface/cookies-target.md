...
>
>如需在使用[!DNL Target][[!DNL Adobe Experience Platform Web SDK]的](https://experienceleague.adobe.com/docs/experience-platform/edge/home.html?lang=zh-Hant){target=_blank}實作中使用之Cookie的相關資訊，請參閱 [!DNL Adobe Experience Platform Web SDK]如果是的話，會使用哪些Cookie？」 在[[!DNL Platform Web SDK概觀指南的常見問題集]](https://experienceleague.adobe.com/docs/experience-platform/edge/web-sdk-faq.html?lang=zh-Hant){target=_blank}中。
>
...
可以使用at.js實作中的`secure`設定，在所有這些Cookie上啟用`secureOnly: true`設定。
...
可以使用at.js實作中的`secure`設定，在所有Cookie上啟用`secureOnly: true`設定。