---
description: Analytics 和 Target 客戶屬性常見問題集和最佳作法。
...
| 問題 | 說明 |
|--- |--- |
| [!UICONTROL 客戶屬性]訂閱限制 | 當您升級至Analytics Premium時，會有24小時的延遲，才會有更多屬性可用。 在這段期間，您可能會看到已達到屬性訂閱上限錯誤。 |
| 同一部裝置上多次登入 | 當使用客戶屬性將客戶個人檔案上傳至資料來源時，Adobe建議使用共用裝置（亦即相同的Experience Cloud ID）的使用者不要使用。 Experience Cloud ID(ECID)在裝置上持續存在。 共用裝置可讓ECID將多個使用者連結至相同的ECID，造成非預期的結果 [!DNL Target]。 **注意：** 對於Mobile，在安裝Mobile應用程式後，ECID會是永久的。您必須重新安裝應用程式，才能產生新的ECID。 若使用網頁，系統會在瀏覽器 Cookie 清除後產生新的 ECID。 |
...
| 該如何開始使用客戶屬性？ | <ol><li>完成佈建程序。如果您是 Analytics 客戶，Adobe 會針對客戶屬性為您完成佈建。如果您只使用Adobe Target而沒有Analytics，請聯絡客戶服務以要求核心服務的布建。</li> <li>與您的 CRM 團隊溝通。瞭解您想要在Analytics和整個Experience Cloud中使用哪些類型的客戶資料。</li><li>實作核心服務。如需導入最新實作的步驟，請參閱[啟用核心服務的解決方案](https://docs.adobe.com/content/help/zh-Hant/core-services/interface/about-core-services/core-services.html)（如需重要資訊，請參閱有關同步客戶ID的章節。）</li></ol> **注意：**[這裡](https://docs.adobe.com/content/help/zh-Hant/core-services/interface/manage-users-and-products/faq.html#concept_13219B4E51784577B6FF78AAA203DE91)提供實作核心服務的管理員常見問題集。 |
...
| 是否需要移轉至 Experience Cloud ID Service？ | 是否需移轉取決於您使用的解決方案。 <ul><li>Adobe Analytics：強烈建議 </li><li>Adobe Target：必須移轉。 </li></ul><br>使用Experience Cloud ID服務可啟用最新的Experience Cloud功能，包括即時觀眾、Adobe Target現代化、Analytics整合和視訊心率追蹤。 <br> 如需詳細資訊，請參 [閱啟用核心服務的解決方案](https://docs.adobe.com/content/help/zh-Hant/core-services/interface/about-core-services/core-services.html)。 <br>**注意：**[Experience Cloud ID服務](https://docs.adobe.com/content/help/zh-Hant/id-service/using/intro/overview.html) ，是舊稱 _Analytics訪客ID服務的現代化實作。_ |
| 客戶屬性功能與 Adobe Audience Manager 有何關係？ | 雖然Audience Manager可以接收資料以執行受眾識別，但無法執行將屬性與歷史行為資料關聯的分析功能。 它也不提供Adobe Analytics中可用的報表、分析和分段功能。 [!UICONTROL People] 可讓不同解決方案的豐富資料繫結在一起，並與單一 ID 建立關聯，以便在 Experience Cloud 中使用。<br>Adobe Target 中，客戶屬性會顯示為個別屬性，可與其他規則結合以建立受眾。共用給 [!UICONTROL People] 服務的受眾是無法修改的完整受眾。 |
