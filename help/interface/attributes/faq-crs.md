...
| 同一部裝置上多次登入 | 使用客戶屬性將客戶設定檔上傳至資料來源時，Adobe 不建議使用者共用相同的裝置 (亦即使用相同的 Experience Cloud ID)。這麼做可能會導致 ECID 服務 (此服務會在裝置上持續運作) 在相同的 Experience Cloud ID 下連結多位使用者，造成 [!DNL Target] 中出現無法預期的結果。**注意：**&#x200B;若使用行動裝置，行動應用程式安裝後 ECID 會永久存在，必須重新安裝應用程式才能產生新的 ECID。若使用網頁，系統會在瀏覽器 Cookie 清除後產生新的 ECID。 |
...
| 客戶屬性功能與 Adobe Audience Manager 有何關係？ | 雖然 Audience Manager 可接收資料以執行受眾識別，但無法執行將屬性繫結至歷史行為資料的分析功能，也無法提供 Adobe Analytics 中可用的報表、分析和細分功能。[!UICONTROL People] 可讓不同解決方案的豐富資料繫結在一起，並與單一 ID 建立關聯，以便在 Experience Cloud 中使用。<br>Adobe Target 中，客戶屬性會顯示為個別屬性，可與其他規則結合以建立受眾。共用給 [!UICONTROL People] 服務的受眾是無法修改的完整受眾。 |
