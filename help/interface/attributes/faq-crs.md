...
| 客戶屬性功能與Adobe Audience Manager有何關係？ | 雖然Audience Manager可接收資料以執行觀眾識別，但無法執行將屬性系結至歷史行為資料的分析功能，或提供Adobe Analytics中可用的報告、分析和區段功能。 [!UICONTROL People] 可讓不同解決方案的豐富資料連結在一起，並與單一ID關聯，以便在Experience Cloud中使用。 <br>在Adobe Target中，客戶屬性會顯示為個別屬性，可與其他規則結合以建立觀眾。 共用給 [!UICONTROL People] service的觀眾是無法修改的完整觀眾。 |
