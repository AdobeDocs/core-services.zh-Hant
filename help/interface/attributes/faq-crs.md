...
| 問題 | 說明 |
|--- |--- |
| 客戶屬性訂閱限制 | 當您升級至Analytics Premium時，會有24小時的延遲，才會有其他屬性可用。 在這段期間，您可能會看到已達到屬性訂閱上限錯誤。 |
...
| **（僅限Adobe Target）** 「同步狀態」欄 **** 代表哪些檔案在「客戶屬性來源」中上傳？ | 按一下特定屬性檔案的「同步狀態」圖示，即可檢視Adobe Target所發佈和同步的記錄數。 `Sync %` 是即時度量，可指定在Adobe Target中同步的設定檔百分比。<br> **注意：** 屬性與Adobe Target同步可能需要24小時。 |
