---
description: 了解如何透過Experience Cloud中的不同應用程式和服務實作DNS預先擷取，以縮短頁面載入時間。
solution: Experience Cloud
title: '搭配不同的應用程式和服務使用DNS預先擷取 '
...
# 搭配不同的應用程式和服務使用DNS預先擷取

實作DNS預先擷取，以縮短不同應用程式和服務的頁面載入時間。
...
## DNS預先擷取和Adobe Experience Cloud應用程式 {#section_202A07F9F79F4ABDA44B98BA1DDCD516}

DNS 預先擷取可自動與頁面上的靜態內嵌連結搭配使用。這也表示自動DNS預先擷取不適用於不同 [!UICONTROL Experience Cloud] 應用程式和服務，因為：

* 每個Experience Cloud應用程式或服務會在頁面載入時動態產生DNS呼叫。
* 發出這些呼叫之前，瀏覽器無法將網域名稱解析為 IP 地址。

不過，您可以透過Experience Cloud應用程式手動實作DNS預先擷取。 做法是將 HTML `<dns-prefetch>` 標記新增至網頁代碼的 `<head>` 區段，如下所示。若正確實作，DNS 預先擷取便有助於節省數毫秒的網頁載時間。

## DNS 預先擷取程式碼範例 {#section_E886F7B2861E48BA9EF3D8B3CE32B345}

下列範例說明如何將DNS預先擷取調用次數設為不同 [!DNL Experience Cloud] 應用程式和服務。 部分預先擷取的調用次數需要提供您的 [!DNL Adobe] 組織 ID 或追蹤伺服器資訊。在以下範例中，*斜體*&#x200B;代碼表示變數的預留位置。您可以使用自己的 [!DNL Adobe] 合作夥伴 ID、客戶代碼或追蹤伺服器資訊等來取代該代碼。
