---
description: 了解如何管理 Adobe Experience Cloud 使用者權限和產品設定檔。了解如何登入 Adobe Admin Console 以及使用 Experience Cloud 的瀏覽器支援。
solution: Admin
title: '使用者和產品管理 '
index: true
...
了解 Experience Cloud 使用者和產品管理的最新功能。
...
管理員不再需要於特定的產品解決方案中管理使用者。Experience Cloud 的使用者和產品管理現在需在 Admin Console 中進行。
...
例如，在 Analytics 中，您可以設定報表工具的集合，例如 Analysis Workspace 和 Report Builder，以及報表套裝、量度、維度等項目。您可以透過將使用者新增至設定檔來授予產品設定檔的權限給使用者。請參閱本頁面的[將 Analytics 存取權指派給產品設定檔](../admin-getting-started/admin-getting-started.md#task_040673FE3E3E429B9531FBCB8B6A4391)。

**委託管理權限**

請參閱本頁面的[委派有限管理權限](../admin-getting-started/admin-getting-started.md#task_3A072C4AA9734BC59FFA7E015271BC7E)。
...
請參閱[使用者權限](https://experienceleague.adobe.com/docs/launch/using/admin/user-permissions.html?lang=zh-Hant#admin)。
