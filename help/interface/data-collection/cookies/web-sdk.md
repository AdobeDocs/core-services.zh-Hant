...
| **kndctr_&lt;ORG_ID>_cluster** | 1800 （30分鐘） | 3-5位元組 | 儲存Edge Network區域以提供目前使用者的請求。 URL路徑會使用地區，這樣Edge Network就能將請求路由至正確的地區。 如果使用者使用不同的IP位址或在不同的工作階段中連線，則請求會再次路由到最近的區域。 |
| **mbox** | 63072000 （2年） | | 目標移轉設定設為true時出現。 它允許網頁SDK設定Target [mbox Cookie](https://developer.adobe.com/target/implement/client-side/atjs/atjs-cookies/)。 |
