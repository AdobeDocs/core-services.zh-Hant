...
如果您最近使用[Experience Platform標籤](https://experienceleague.adobe.com/en/docs/experience-platform/tags/home)實作CX Enterprise，表示您已為[客戶屬性](../services/overview.md)和CX Enterprise [對象](../services/audiences/overview.md)完成設定。 您也可以在[Admin Console](../administration/admin-console.md)中管理使用者和產品。
...
**[!UICONTROL Admin Console]**&#x200B;連結可在CX Enterprise功能表導覽中使用，以管理使用者和產品授權。
...
檢視CX Enterprise中的[連結帳戶](https://experienceleague.adobe.com/en/docs/core-services/interface/administration/organizations)
...
1. [在[!UICONTROL Admin Console]中管理使用者和產品](../administration/admin-console.md)。

**相關：** [Analytics和Target — 同步化客戶ID](#sync-ids) （在此頁面上）

## 實作 [!UICONTROL CX Enterprise ID Service]
...
>[!NOTE]
>
>如需完整的CX Enterprise ID服務說明（先前稱為訪客ID），請參閱[CX Enterprise Identity Service概觀](https://experienceleague.adobe.com/docs/id-service/using/intro/overview.html#intro)。
...
| 任務 | 說明 |
| --- | --- |
| [實作適用於Analytics的CX Enterprise ID服務](https://experienceleague.adobe.com/en/docs/analytics/implementation/id/overview) | Adobe 也建議設定其他[客戶 ID](https://experienceleague.adobe.com/en/docs/id-service/using/reference/authenticated-state)。 這些ID與每個訪客相關聯，以啟用CX Enterprise中現有和未來的功能。 |
...
CX Enterprise [!UICONTROL Audiences]介面可讓您建立對象、組合現有對象以建立複合對象，以及檢視所有共用對象。
