...
## CX Enterprise的Adobe Analytics和Adobe Target需求 {#experience-cloud-requirements}

使用CX Enterprise的[!DNL Analytics]和[!DNL Adobe Target]需求：
...
1. 確定您具有適當的 Adobe Analytics 或 Adobe Target SKU。
   ...
   * **Adobe Target：** Standard 或 Premium 版。
     ...
     >[!NOTE]
     >
     >若使用 [!DNL Target]，需從 `mbox.js` 移轉至 at.js。 請參閱[從at.js 1. x升級為at.js 2. x](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/upgrading-from-atjs-1x-to-atjs-20.html)。

1. 在[!UICONTROL Admin Console]中[管理使用者和產品](../administration/admin-console.md)。

**相關：** [Analytics和Target — 同步化客戶ID](#sync-ids) （在此頁面上）

## 實作[!UICONTROL CX Enterprise ID服務]

[!UICONTROL CX Enterprise ID服務]提供跨應用程式整合的通用ID。 它提供跨網域訪客身分識別，以及根據透過[!DNL Customer Attributes]上傳的CRM資料跨裝置/瀏覽器鎖定目標和個人化的路徑。

啟用CX企業核心服務最簡單的方法，就是透過[!UICONTROL Experience Platform Launch]中的[CX Enterprise ID Service擴充功能](https://experienceleague.adobe.com/docs/experience-platform/tags/extensions/adobe/id-service/overview.html?lang=zh-Hant)，為Analytics和Adobe Target自動啟用。

>[!NOTE]
>
>如需完整的CX Enterprise ID服務說明（先前稱為訪客ID），請參閱[CX Enterprise Identity Service概觀](https://experienceleague.adobe.com/docs/id-service/using/intro/overview.html?lang=zh-Hant#intro)。


**未使用[!UICONTROL Experience Platform標籤]？**

如果您沒有使用[!UICONTROL Experience Platform標籤]，請透過JavaScript部署(`VisitorAPI.js`)手動實作ID服務，如下所示：

| 任務 | 說明 |
| --- | --- |
| [實作適用於Analytics的CX Enterprise ID服務](https://experienceleague.adobe.com/zh-hant/docs/analytics/implementation/id/overview) | Adobe 也建議設定其他[客戶 ID](https://experienceleague.adobe.com/zh-hant/docs/id-service/using/reference/authenticated-state)。 這些ID與每個訪客相關聯，以啟用CX Enterprise中現有和未來的功能。 |
...
如需在[Android](https://experienceleague.adobe.com/docs/mobile-services/android/overview.html?lang=zh-Hant)和[Enterprise ID](https://experienceleague.adobe.com/docs/mobile-services/ios/overview.html?lang=zh-Hant)行動應用程式中設定其他客戶ID的語法範例，請參閱&#x200B;*CX iOS服務™1&rbrace;一節。*

### 啟用歷史資料的屬性

訪客登入後，即可使用客戶屬性資料。 如果您尚未實作ID Service，而且過去一直在prop或eVar中追蹤客戶ID，您可以請求將歷史登入資料傳送至CX Enterprise的程式。 此程序可讓您立即開始使用客戶屬性。
...
* 建議您在[!UICONTROL Adobe Target]標籤中新增[Experience Platform擴充功能](https://experienceleague.adobe.com/docs/experience-platform/tags/extensions/adobe/target-v2/overview.html?lang=zh-Hant)，以便讓資料庫擷取作業自動執行。 您也可以使用[!UICONTROL Enterprise ID]標籤為Adobe Target （和其他應用程式）設定[CX Experience Platform服務擴充功能](https://experienceleague.adobe.com/docs/experience-platform/tags/extensions/adobe/id-service/overview.html?lang=zh-Hant)。 Adobe Target需要[!UICONTROL CX Enterprise ID服務]更新&#x200B;**&#x200B;**&#x200B;才能使用People服務。
...
1. 確認Analytics請求包含`mid`引數(CX Enterprise ID)。 在寬限期內 (如果已啟用)，您應該也會看到 `aid` 參數 (Analytics 訪客 ID)。
...
mbox要求中的![CX Enterprise ID](../assets/mbox_request.png)
...
如需詳細資訊，請參閱[客戶屬性](https://experienceleague.adobe.com/zh-hant/docs/core-services/interface/services/customer-attributes/attributes)。
...
如果您利用 Adobe [!DNL CX Enterprise] 中的客群資料即時設定及其他核心服務，則使用這些服務可能會影響您資料所在的資料中心 (和國家/地區)。 具體來說，因為[!DNL CX Enterprise]使用Audience Manager，[!UICONTROL People]服務中使用的資料必須位於美國的Audience Manager伺服器。
...
* 如未封鎖第三方 cookie，則會設置 demdex.net cookie。 `AMCV_###@AdobeOrg`第一方Cookie一律以CX Enterprise ID服務設定。
