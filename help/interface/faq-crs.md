...
Adobe Analytics 和 Adobe Target 中[!UICONTROL 客戶屬性]的常見問題和最佳實務。
...
| 問題 | 說明 |
|--- |--- |
| [!UICONTROL 客戶屬性]訂閱限制 | 升級至 Analytics Premium 後，必須等待 24 小時才能使用更多屬性。在這段期間，您可能會看到已達到[!UICONTROL 性訂閱上限]錯誤。 |
...
| **(僅限 Adobe Target)** 我是否可以預先載入或上傳 Adobe Target 從未看過的客戶資料？ | 是。訪客第一次向 Adobe Target 提出請求時，系統會從「客戶屬性」擷取與 Adobe 有關的現有資訊，並使用這些資料進行目標鎖定。**注意：**&#x200B;如果訪客是第一次與 Adobe Target 互動，則擷取此資料可能需花費最長達 20 分鐘。 |
| **(僅限 Adobe Target)**&#x200B;我是否可藉由結合客戶屬性資料和共用的對象資料來建立超級對象？ | 不會。共用的對象資料是已完成的對象。 |
...
| **(僅限 Adobe Target)** 對於已上傳至客戶屬性來源的檔案而言，**[!UICONTROL 同步狀態]**&#x200B;欄代表什麼？ | 您可以針對特定屬性檔案選取「同步狀態」圖示，以檢視Adobe Target發佈和同步的記錄數。 `Sync %` 是即時量度，可指明 Adobe Target 中已同步的個人資料百分比。<br> **注意：**&#x200B;與 Adobe Target 同步屬性可能需花費最多 24 小時。 |
